...
OWNER_ID: Final[int] = int(os.getenv("OWNER_ID", "0"))
...
async def process_lesson_entry(message: Message, state: FSMContext):
    ...
    try:
        ...
        rest = parts[1].split(",") if len(parts) > 1 else [parts[1] if len(parts) > 1 else ""]
        ...
        await message.answer(f"✅ Урок {num}. <b>{subject}</b> добавлен! Продолжайте или /done")
...
async def process_quiz_correct(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(f"❌ Введите номер от 1 до {len(await state.get_data().get('options', []))}!")
